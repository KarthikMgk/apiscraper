...
class APIFinder:
    ...
    def start(self):
        ...
        if self.url:
            ...
            self.browser = Browser(
                "chromedriver/chromedriver", "browserproxy/bin/browsermob-proxy", self.harDirectory, cookies=self.cookies)
            if self.searchString is not None:
                print("Searching URL "+self.url +
                      " for string "+self.searchString)
        ...
        # Scan directory of har files
            ...
            harParser = HarParser(
                self.harDirectory, self.searchString, self.removeParams)
    ...
    def findInternalURLsInText(self, text, currentUrl, allFoundURLs):
        newUrls = []
        regex = re.compile(
            r'(https?://[\w]+\.)(com|org|biz|net)((/[\w]+)+)(\.[a-z]{2,4})?(\?[\w]+=[\w]+)?((&[\w]+=[\w]+)+)?', re.ASCII)
    ...
    def crawlingScan(self, url, apiCalls=[], allFoundURLs=[]):
        ...
        harParser = HarParser(
            self.harDirectory, searchString=self.searchString, removeParams=self.removeParams)
